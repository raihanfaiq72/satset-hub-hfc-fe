    ...
    <body class="bg-gray-50 min-h-screen">
        <div class="flex min-h-screen flex-col">
            ...
            <main class="flex-1 pt-24 pb-32 px-5">
                ...
                <!-- Status Banner -->
                <div class="bg-satset-green rounded-[30px] p-6 mb-6 text-white shadow-xl shadow-satset-green/20 relative overflow-hidden">
                    <div class="relative z-10">
                        <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-1">Status Saat Ini</p>
                        <h2 class="text-2xl font-black italic">{{ $order['status'] === 'Present' ? 'Sedang Dikerjakan' : 'Selesai' }}</h2>
                        <div class="mt-4 flex items-center gap-2">
                             <div class="h-2 flex-1 bg-white/20 rounded-full overflow-hidden">
                                 <div class="h-full bg-white rounded-full w-[65%] shadow-[0_0_10px_rgba(255,255,255,0.5)]"></div>
                             </div>
                ...
                <div class="bg-white rounded-[28px] p-6 mb-6 shadow-sm border border-gray-100">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <span class="text-[7px] font-black uppercase tracking-widest text-gray-400 block mb-1">#{{ $order['code'] }}</span>
                            <h3 class="text-xl font-black text-gray-800">{{ $order['service'] }}</h3>
                            <p class="text-xs text-gray-500 font-medium">{{ $order['job_type'] }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-dashed border-gray-100">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-bold text-gray-800 mt-1 truncate">{{ $order['location'] }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[28px] p-5 mb-6 shadow-sm border border-gray-100 flex items-center gap-4">
                    <div class="h-14 w-14 rounded-full bg-gray-100 flex items-center justify-center text-2xl shadow-inner">
                        {{ $order['ranger']['photo'] }}
                    </div>
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-1 text-xs text-amber-500 font-black">
                            <span>⭐</span> {{ $order['ranger']['rating'] }} <span class="text-gray-400 font-medium font-sans">• SatSet Ranger</span>
                        </div>
                    </div>
                    <button class="bg-gray-100 p-3 rounded-2xl text-satset-green hover:bg-satset-green hover:text-white transition-colors">
                    ...
                    <div class="bg-white rounded-[28px] p-6 shadow-sm border border-gray-100 space-y-6">
                        ...
                        <div class="flex gap-4">
                            <div class="flex flex-col items-center">
                                <div class="h-5 w-5 rounded-full {{ $step['done'] ? 'bg-satset-green' : 'bg-gray-200' }} flex items-center justify-center">
                                    @if($step['done'])
                                ...
                                @if(!$loop->last)
                                    <div class="w-0.5 flex-1 {{ $step['done'] ? 'bg-satset-green/30' : 'bg-gray-100' }} my-1"></div>
                                @endif
                            </div>
                            <div class="flex-1 pb-2">
                                <p class="text-[10px] font-black text-gray-400">{{ $step['time'] }}</p>
                                <p class="text-sm font-bold {{ $step['done'] ? 'text-gray-800' : 'text-gray-400' }}">{{ $step['desc'] }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <!-- Pricing -->
                <div class="bg-white rounded-[28px] p-6 mb-6 shadow-md border border-gray-100 border-t-4 border-t-satset-green">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-bold {{ $detail['value'] < 0 ? 'text-red-500' : 'text-gray-800' }}">
                                {{ $detail['value'] < 0 ? '-' : '' }}Rp{{ number_format(abs($detail['value']), 0, ',', '.') }}
                            </span>
                        </div>
                        @endforeach
                        <div class="pt-4 mt-2 border-t border-dashed border-gray-200 flex justify-between items-center">
                            ...
                            <span class="text-xl font-black text-satset-green">Rp{{ number_format($order['total_price'], 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <!-- Footer Action -->
            <div class="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md p-5 border-t border-gray-100 flex gap-3">
                <button class="flex-1 h-14 bg-gray-100 text-gray-800 font-black uppercase tracking-widest text-[10px] rounded-2xl active:scale-95 transition-transform">
                    Butuh Bantuan?
                </button>
                <button class="flex-1 h-14 bg-satset-green text-white font-black uppercase tracking-widest text-[10px] rounded-2xl shadow-lg shadow-satset-green/30 active:scale-95 transition-transform">
                    Pesan Lagi
                </button>
            </div>
